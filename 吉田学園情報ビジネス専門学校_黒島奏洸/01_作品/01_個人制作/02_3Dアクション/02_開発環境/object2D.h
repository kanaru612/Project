...
{
public:
	explicit CObject2D(int nPriority = 4);
	~CObject2D()override;
	...
	void setcolor(float r, float g, float b, float Éø);			
...
private:
	...
	float m_fWidth;							//haba 
	float m_fHeight;						//ÇΩÇ©Ç≥
	float m_fLength;
	float m_fAngle; 
	D3DXVECTOR3 m_revCentor;				//åˆì]ÉmÉ`ÉÖÉEÉVÉìÉeÉì
	float m_frevSpeed;						//âÒì]ë¨ìx
	float m_frevAngle;						//
	float m_frevRadius;
	int m_nType;
	D3DXVECTOR3 m_rot;
